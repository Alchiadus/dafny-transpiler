...
export default class Generator {
  ...
  generateInitializationCorrectnessMethod(program, programComponents) {
    ...
    initializationCorrectnessMethod.annotations = [];
  ...
  generateLocalCorrectnessMethods(programComponents) {
    ...
    for (let programComponent of programComponents) {
      code += this.generateLocalCorrectnessMethodsForProgramComponent(programComponent);
    }
    return code;
  }

  /**
   * Generates source code for the local correctness methods of a program component.
   *
   * @private
   * @param  {Object}   programComponent  the parallel program component
   * @return {String}                     a string containing the generated source code
   */
  generateLocalCorrectnessMethodsForProgramComponent(programComponent) {
    let code = '';
    let assertions = programComponent.assertions;
    for (let i = 0; i < assertions.length; i++) {
      const assertion = assertions[i];
      const localCorrectnessMethod = {};
      localCorrectnessMethod.name = `LocalCorrectness${programComponent.name}${i + 1}`;
      localCorrectnessMethod.annotations = programComponent.annotations;
      localCorrectnessMethod.preconditions = [assertion.precondition];
      localCorrectnessMethod.postconditions = [assertion.postcondition];
      localCorrectnessMethod.statements = assertion.statements;
      code += this.generateMethod(localCorrectnessMethod);
      code += this.newline();
    }
    return code;
  }
  ...
  generateFinalizationCorrectnessMethod(program, programComponents) {
    ...
    finalizationCorrectnessMethod.annotations = [];
    ...
    for (let assertion1 of programComponent1.assertions) {
      for (let assertion2 of programComponent2.assertions) {
        for (let assertion of [assertion2.precondition, assertion2.postcondition]) {
          ...
          globalCorrectnessMethodPrecondition.annotations = programComponent1.annotations;
  ...
  generateMethod(method) {
    ...
    if (method.annotations.length > 0) {
      for (let annotation of method.annotations) {
        code += `${this.tab(2)}${annotation}${this.newline()}`;
      }
    }
