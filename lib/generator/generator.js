...
export default class Generator {
  ...
  generateGlobalCorrectnessMethod(programComponent1, programComponent2, counter) {
    let code = '';
    code += `${this.tab()}// A: ${programComponent1.name} - S: ${programComponent2.name} (preconditions)${this.newline()}`;
    ...
    code += this.newline();
    code += `${this.tab()}// A: ${programComponent1.name} - S: ${programComponent2.name} (postconditions)${this.newline()}`;
    ...
    code += this.newline();
    if (programComponent2.assertions.length > 0) {
      code += `${this.tab()}// A: ${programComponent1.name} - S: ${programComponent2.name} (assertions)${this.newline()}`;
      const globalCorrectnessMethodAssertion = {};
      globalCorrectnessMethodAssertion.name = `GlobalCorrectness${++counter}`;
      globalCorrectnessMethodAssertion.preconditions = programComponent1.preconditions;
      globalCorrectnessMethodAssertion.preconditions = globalCorrectnessMethodAssertion.preconditions.concat(programComponent2.assertions);
      globalCorrectnessMethodAssertion.postconditions = programComponent2.assertions;
      globalCorrectnessMethodAssertion.statements = programComponent1.statements;
      code += this.generateMethod(globalCorrectnessMethodAssertion);
      code += this.newline();
    }
    return {
      _code: code,
