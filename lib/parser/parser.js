...
export default class Parser {
  ...
  extractProgram(sourceCode) {
    ...
    for (let methodName of methodNames) {
      const methodSourceCode = this.extractMethodSourceCode(methodName, sourceCode);
      let methodAnnotations = this.extractSourceCodeLines(methodSourceCode.annotations);
      let methodBody = this.extractSourceCodeLines(methodSourceCode.body);
      let methodAssertions = [];
      let methodAssertionsRegExp = /^\s*assert([\s\S]*?)\/\/\s*#POG$\r?\n([\s\S]*?)\r?\n^\s*assert([\s\S]*?)\/\/\s*#QOG$/gm;
      let match;
      while ((match = methodAssertionsRegExp.exec(methodSourceCode.body)) !== null) {
      ...
      const programComponentBodies = /\/\/ #parallel([\s\S]*?)\/\/ #endparallel/g.exec(methodSourceCode.body);
      if (programComponentBodies) {
        ...
        programComponents.push(new ProgramComponent(methodName, methodAnnotations, methodBody, methodAssertions));
      }
    }
  ...
  /**
   * Extracts given the method name the method annotations and body from a piece of Dafny source code.
   *
   * @private
   * @param  {String} methodName the method name to extract the method annotations and body from
   * @param  {String} sourceCode the source code to extract the method annotations and body from
   * @return {String}            the method source code
   */
  extractMethodSourceCode(methodName, sourceCode) {
    const methodSourceCode = new RegExp(`method ${methodName}\\(\\)([\\s\\S]*?){([\\s\\S]*?)} \\/\\/ #endmethod ${methodName}`, 'g').exec(sourceCode);
    return {
      annotations: methodSourceCode[1],
      body: methodSourceCode[2]
    };
  }
  ...
  extractSourceCodeLines(sourceCode) {
    let sourceCodeLines = sourceCode.split(/\r?\n/g);
