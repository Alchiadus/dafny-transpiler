...
export default class Parser {
  ...
  extractMethods(sourceCode) {
    ...
    while ((match = methodRegEx.exec(sourceCode)) !== null) {
      ...
      method.assertions = this.extractAssertions(match[3]);
  ...
  /**
   * Extracts all assertions from a snippet of Dafny source code.
   *
   * @private
   * @param  {String}        snippet the snippet to extract the assertions from
   * @return {Array<String>}         an array containing the assertions
   */
  extractAssertions(snippet) {
    return this.execAll(/assume|assert|invariant (.*?);\n/g, snippet);
  }
